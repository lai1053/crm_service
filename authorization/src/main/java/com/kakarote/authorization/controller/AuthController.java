...
public class AuthController {
    ...
    @RequestMapping(value = "/ossLogin")
    @ApiOperation(tags = "oss用户登录", httpMethod = "POST", value = "/ossLogin")
    @ParamAspect
    public Result ossLogin(@Valid @RequestBody AuthorizationUser user, HttpServletResponse response,HttpServletRequest request) {
        //String userName = request.getParameter("userName");
        String userName = StrUtil.trimToNull(user.getUsername());

        return loginService.ossLogin(userName, response, request);
    }
}
